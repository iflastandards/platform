...
describe('VocabularyTable Component', () => {
  ...
  describe('Basic Rendering', () => {
    it('renders vocabulary table with basic props', () => {
      ...
      expect(
        screen.getByText('A term used for testing purposes'),
      ).toBeInTheDocument();
      expect(
        screen.getByText('A test vocabulary for unit testing'),
      ).toBeInTheDocument();
    });
    ...
    it('hides title by default', () => {
      render(<VocabularyTable {...basicProps} />);
      expect(
        screen.queryByRole('heading', { name: 'Test Vocabulary' }),
  ...
  describe('URI Generation', () => {
    ...
    it('generates slug URIs when configured', () => {
      ...
      expect(
        screen.getByText('(isbdm:test-vocab#test-term)'),
      ).toBeInTheDocument();
      expect(
        screen.getByText('(isbdm:test-vocab#another-term)'),
      ).toBeInTheDocument();
    });
  ...
  describe('Configuration Options', () => {
    ...
    it('uses custom filter placeholder', () => {
      render(
        <VocabularyTable {...basicProps} filterPlaceholder="Search terms..." />,
      );
  ...
  describe('Accessibility', () => {
    it('has proper ARIA labels', () => {
      ...
      expect(
        screen.getByLabelText('Filter vocabulary terms'),
      ).toBeInTheDocument();
  ...
  describe('Integration with Site Config', () => {
    ...
    it('overrides site config with props', () => {
      render(
        <VocabularyTable {...basicProps} prefix="custom" startCounter={5000} />,
      );
  ...
  describe('Dark Mode Support', () => {
    it('applies dark mode styles when theme is dark', () => {
      ...
      expect(
        container.querySelector('.vocabulary-table') || container.firstChild,
      ).toBeInTheDocument();
    });
  });
});
