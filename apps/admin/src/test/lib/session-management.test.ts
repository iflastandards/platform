...
describe('Session Management', () => {
  ...
  describe('Cross-Site Session Sharing', () => {
    it('should share session between admin and portal domains', async () => {
      ...
      const portalRequest = await fetch('http://localhost:4200/api/auth/session', {
        headers: {
          Origin: 'http://localhost:3000',
        },
        credentials: 'include',
      ...
      expect(portalRequest.headers.get('Access-Control-Allow-Credentials')).toBe('true');
    });
    ...
    it('should handle production domain configuration', async () => {
      ...
      const response = await fetch('https://admin.iflastandards.info/api/auth/session', {
        headers: {
      ...
      expect(response.headers.get('Access-Control-Allow-Origin')).toBe('https://www.iflastandards.info');
    });
  });
  ...
  describe('Session Security', () => {
    it('should validate session tokens are httpOnly and secure in production', async () => {
      ...
      const mockHeaders = new Headers({
        'Set-Cookie': 'next-auth.session-token=token; HttpOnly; Secure; SameSite=Lax; Path=/',
      });
  ...
  describe('Inactivity Logout', () => {
    it('should track user activity and maintain session', () => {
      ...
      const shouldMaintainSession = Date.now() - lastActivity < inactivityTimeout;
