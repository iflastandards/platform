...
test.describe('Environment Configuration Tests', () => {
  ...
  test('should have root .env file with valid DOCS_ENV', async () => {
    ...
    expect(
      ...
      'Root .env file should exist for local development',
    ).toBe(true);
    ...
    expect(
      validValues,
      `DOCS_ENV value '${docsEnvValue}' should be valid`,
    ).toContain(docsEnvValue);
  });
  ...
  test('should validate all sites have required environment files', async () => {
    ...
    for (const site of sites) {
      ...
      expect(
        fs.existsSync(siteEnvPath),
        `${site} should have .env.site file`,
      ).toBe(true);

      // Verify .env.site.local exists
      expect(
        fs.existsSync(siteLocalEnvPath),
        `${site} should have .env.site.local file`,
      ).toBe(true);
  ...
  test('should validate portal has required environment files', async () => {
    ...
    // Verify base .env.site exists
    expect(
      fs.existsSync(portalEnvPath),
      'portal should have .env.site file',
    ).toBe(true);

    // Verify .env.site.local exists
    expect(
      fs.existsSync(portalLocalEnvPath),
      'portal should have .env.site.local file',
    ).toBe(true);
