...
export async function seedClerkAuth(context: BrowserContext, email: string) {
  ...
  if (!testUser) {
    throw new Error(
      `Clerk test user with email ${email} not found. Available users: ${CLERK_TEST_USERS.map((u) => u.email).join(', ')}`,
    );
  }
  ...
  try {
    ...
    await context.storageState({
      path: `playwright-state-${testUser.role}.json`,
    });
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    throw new Error(
      `Failed to authenticate Clerk user ${email}: ${errorMessage}`,
    );
  } finally {
    await page.close();
  }
