...
export function VocabularyTable({
  ...
  const defaults =
    (siteConfig.customFields?.vocabularyDefaults as VocabularyDefaults) || {};
  ...
  const resolvedShowLanguageSelector =
  ...
  const resolvedDefaultLanguage =
    defaultLanguage || defaults.defaultLanguage || currentLocale;
  ...
  const csvValidation = useMemo<ValidationResult | null>(() => {
    ...
    if ((csvFile && loadedCsvData.length > 0) || (preferCsvData && csvData)) {
      const dataToValidate =
        csvFile && loadedCsvData.length > 0 ? loadedCsvData : csvData;
  ...
  const currentLocalizedConcepts = useMemo(() => {
    return finalConceptsArray.map((concept) => ({
      ...
      definition: getLocalizedText(
        concept.definition,
        currentLanguage,
        resolvedDefaultLanguage,
      ),
      scopeNote: getLocalizedText(
        concept.scopeNote,
        currentLanguage,
        resolvedDefaultLanguage,
      ),
      notation: getLocalizedText(
        concept.notation,
        currentLanguage,
        resolvedDefaultLanguage,
      ),
      example: getLocalizedText(
        concept.example,
        currentLanguage,
        resolvedDefaultLanguage,
      ),
      changeNote: getLocalizedText(
        concept.changeNote,
        currentLanguage,
        resolvedDefaultLanguage,
      ),
      historyNote: getLocalizedText(
        concept.historyNote,
        currentLanguage,
        resolvedDefaultLanguage,
      ),
      editorialNote: getLocalizedText(
        concept.editorialNote,
        currentLanguage,
        resolvedDefaultLanguage,
      ),
      altLabel: getLocalizedText(
        concept.altLabel,
        currentLanguage,
        resolvedDefaultLanguage,
      ),
      allAltLabels: getAllLocalizedText(
        concept.altLabel,
        currentLanguage,
        resolvedDefaultLanguage,
      ),
  ...
  const conceptsWithUris = useMemo(() => {
    return currentLocalizedConcepts
      .map((concept, index) => {
        ...
        const conceptUri =
          concept.originalConcept.uri ||
          `${resolvedPrefix}:${resolvedVocabularyId}#${identifier}`;
  ...
  const hasAdditionalDetails = (concept: {
    notation?: string;
    example?: string;
    changeNote?: string;
    historyNote?: string;
  ...
  const localizedScopeNote = getLocalizedText(
    scopeNote,
    currentLanguage,
    resolvedDefaultLanguage,
  );
  ...
  return (
    <div
      className={styles.vocabularyContainer}
      id={`vocabulary-${resolvedVocabularyId}`}
    >
      ...
      <div className={styles.vocabularyHeader}>
        ...
        {resolvedShowLanguageSelector &&
          resolvedAvailableLanguages.length > 1 && (
            <div className={styles.languageSelector}>
              <label
                htmlFor={`language-select-${resolvedVocabularyId}`}
      ...
      {resolvedShowCSVErrors &&
        csvValidation &&
        csvValidation.issues.length > 0 && (
          <details className={styles.errorBadgeContainer}>
            <summary
              className={`${styles.errorBadge} ${csvValidation.issues.some((issue) => issue.type === 'error') ? styles.errorBadgeError : styles.errorBadgeWarning}`}
            >
              <span className={styles.errorBadgeIcon}>
                {csvValidation.issues.some((issue) => issue.type === 'error')
                  ? '❌'
                  : '⚠️'}
              </span>
              <span className={styles.errorBadgeText}>
                {csvValidation.issues.length} validation issue
                {csvValidation.issues.length > 1 ? 's' : ''} found
              </span>
      ...
      {!csvLoading && !csvError && (
        <>
          ...
          <div
            className={`${styles.tableContainer} ${isDarkTheme ? styles.darkMode : ''}`}
          >
            <div className={styles.headerRow}>
              {hasAnyAdditionalDetails && (
                <div className={styles.headerDetails}>Detail</div>
              )}
                  ...
                  >
                    ...
                    <div className={styles.valueCell}>
                      <p className={styles.valueText}>
                        {entry.value}
                        {entry.allAltLabels &&
                          entry.allAltLabels.length > 0 && (
                            <span className={styles.altLabel}>
                              {' '}
                              ({entry.allAltLabels.join(', ')})
                            </span>
                          )}
                        {entry.uri && resolvedShowURIs && (
                          <span className={styles.uriText}>
                            <br />({entry.uri})
                          </span>
                        )}
                      </p>
