...
describe('Page Template Generator Integration Tests', () => {
  ...
  describe('Complete Generation Process', () => {
    it('should generate all required files and pass validation', async () => {
      ...
      } = require('./utils/sidebar-reference-extractor');
      extractSidebarReferencesFromFile.mockResolvedValue(mockSidebarReferences);
      expandAutogeneratedReferences.mockResolvedValue(mockExpandedReferences);

      // Mock file existence checker to initially report missing files, then none after generation
      const {
        checkFilesExist,
        generateMissingFilesReport,
      } = require('./utils/file-existence-checker');
      ...
      checkFilesExist.mockResolvedValueOnce(missingFiles);
      generateMissingFilesReport.mockReturnValueOnce('Found 2 missing files');

      // Second call (after generation) - report no missing files
      checkFilesExist.mockResolvedValueOnce([]);
      generateMissingFilesReport.mockReturnValueOnce(
  ...
  describe('Validation Process', () => {
    it('should identify missing files and generate a report', async () => {
      ...
      checkFilesExist.mockResolvedValue(missingFiles);
      generateMissingFilesReport.mockReturnValue('Found 2 missing files');
      ...
      expect(extractSidebarReferencesFromFile).toHaveBeenCalledWith(
        'standards/test-integration/sidebars.ts',
      );
      expect(expandAutogeneratedReferences).toHaveBeenCalledWith(
      ...
      expect(generateMissingFilesReport).toHaveBeenCalledWith(missingFiles);
    });
  });
