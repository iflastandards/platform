...
test.describe('Site Management Workflow with newtest', () => {
  ...
  test('should handle direct site access workflow', async ({ page }) => {
    ...
    await page.goto('http://localhost:3007/site/newtest');
  ...
  test('should handle error states gracefully', async ({ page }) => {
    ...
    await page.goto('http://localhost:3007/dashboard/nonexistent-site');
  ...
  test('should display proper page metadata', async ({ page }) => {
    ...
    // Check page title
    await expect(page).toHaveTitle(/New Test Site.*Management/);
    
    // Check meta description if present
    const metaDescription = page.locator('meta[name="description"]');
    if (await metaDescription.count() > 0) {
      await expect(metaDescription).toHaveAttribute('content', expect.stringMatching(/management|admin/i));
    }
  });
});
