...
import { mockSiteData, mockGitHubData, createMockResponse, createMockError } from '../fixtures/mockData';
...
export const mockApiResponses = {
  ...
  getSite: (siteKey: string) => {
    ...
    return site ? createMockResponse(site) : createMockError('Site not found', 404);
  },
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  updateSite: (siteKey: string, data: any) => {
    const site = mockSiteData[siteKey as keyof typeof mockSiteData];
    return site ? createMockResponse({ ...site, ...data }) : createMockError('Site not found', 404);
  },
  
  // GitHub API endpoints
  getRepoInfo: () => createMockResponse({
    name: 'standards-dev',
  ...
  getContent: (path: string) => createMockResponse({
    path,
    content: `# Test Content\n\nThis is mock content for ${path}`,
    lastModified: new Date().toISOString(),
  }),
  updateContent: (path: string, content: string) => createMockResponse({
    path,
    content,
    lastModified: new Date().toISOString(),
    success: true,
  ...
  getUserPermissions: (siteKey: string) => createMockResponse({
    siteKey,
...
export const mockApiCall = (endpoint: string, response: any, status = 200) => {
  global.fetch = vi.fn().mockResolvedValueOnce(createMockResponse(response, status));
};

export const mockApiError = (endpoint: string, message: string, status = 500) => {
  global.fetch = vi.fn().mockResolvedValueOnce(createMockError(message, status));
};
