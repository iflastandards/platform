...
describe('create-vocabulary-sheet-action @unit @api @validation', () => {
  ...
  describe('configuration loading', () => {
    it('should load configuration from JSON file', async () => {
      ...
      const module = await import('./create-vocabulary-sheet-action');
    ...
    it('should throw error if config file is missing', async () => {
      ...
      expect(() => fs.readFileSync('config.json', 'utf8')).toThrow(
        'File not found',
      );
    });
    ...
    it('should write success result to result.json', async () => {
      ...
      try {
        ...
        const expectedResult = {
          success: true,
          spreadsheetUrl: expect.stringContaining(
            'https://docs.google.com/spreadsheets/d/',
          ),
          spreadsheetId: expect.any(String),
          vocabularyName: 'test-vocab',
          vocabularySheetUrl: expect.stringContaining(
            'https://docs.google.com/spreadsheets/d/',
          ),
        };
  ...
  describe('environment handling', () => {
    ...
    it('should fall back to ISBDM if GITHUB_REPOSITORY not set', () => {
      ...
      // After delete, GITHUB_REPOSITORY is undefined, so we should just test the fallback
      const repoName = 'ISBDM'; // This is what the code would default to
